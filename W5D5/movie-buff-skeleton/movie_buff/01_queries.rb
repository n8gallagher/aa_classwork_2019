...
def harrison_ford
  ...
  Movie.joins(:actors).where(actors: {name: 'Harrison Ford'}).joins(:castings).where('castings.ord > ?', 1).select(:id, :title).distinct()
end
...
Movie.joins(:actors).group(:id, :title).select(:id, :title).order("COUNT(actor_id) DESC").limit(3)
...
def directed_by_one_of(them)
  ...
  Movie.joins(:actors).where(actors: { name: them }).distinct.select(:id, :title)
end
